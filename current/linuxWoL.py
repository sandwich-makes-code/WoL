...
def save_config(qemu_binary, ram_size, cpu_cores, exit_action, vm_dir=None, custom_iso_path=None, custom_driver_iso_path=None, optional_iso_path=None, display_size="1920x1080"):
    config_data = {
        ...
        "optional_iso_path": optional_iso_path or "",
    ...
    if vm_dir is not None or custom_iso_path or custom_driver_iso_path or optional_iso_path:
        registry_entry = {
...
def register_saved_vm(entry):
    ...
    normalized = {
        ...
        "optional_iso_path": entry.get("optional_iso_path") or "",
...
def get_saved_vm_entries():
    ...
    for disk_path in dict.fromkeys(legacy_paths):
        ...
        entries.append({
            ...
            "optional_iso_path": cfg.get("optional_iso_path", ""),
...
class WoLGui:
    def __init__(self, root):
        ...
        self.optional_iso_var = tk.StringVar(value=cfg.get("optional_iso_path", ""))
    ...
    def build_ui(self):
        ...
        opt_iso_row = tk.Frame(settings_block, bg=GUI_THEME["panel"])
        opt_iso_row.pack(fill="x", pady=(12, 0))
        tk.Label(opt_iso_row, text="Optional ISO Mount", width=18, fg=GUI_THEME["white"], bg=GUI_THEME["panel"], anchor="w", font=("Segoe UI", 10)).pack(side="left")
        opt_iso_entry = tk.Entry(opt_iso_row, textvariable=self.optional_iso_var, width=24, bg=GUI_THEME["white"], fg=GUI_THEME["black"], bd=0, insertbackground=GUI_THEME["black"])
        opt_iso_entry.pack(side="right")
        choose_opt_iso_btn = tk.Button(
            opt_iso_row,
            text="Browse",
            bg=GUI_THEME["blue_light"],
            fg=GUI_THEME["black"],
            font=("Segoe UI", 9, "bold"),
            bd=0,
            command=self.choose_optional_iso,
        )
        choose_opt_iso_btn.pack(side="right", padx=(8, 0))
    ...
    def choose_optional_iso(self):
        selected_optional_iso = filedialog.askopenfilename(title="Select Optional ISO Mount", filetypes=[("ISO files", "*.iso"), ("All files", "*.*")], initialdir=os.path.expanduser("~/Downloads"))
        if selected_optional_iso:
            self.optional_iso_var.set(selected_optional_iso)
    ...
    def launch_vm(self):
        ...
        optional_iso_path = self.optional_iso_var.get().strip()
        use_optional_iso = bool(optional_iso_path)
        ...
        if use_optional_iso and not os.path.exists(optional_iso_path):
            self.root.after(0, lambda: messagebox.showerror("Missing Optional ISO", f"Optional ISO not found:\n{optional_iso_path}"))
            return
        ...
        if use_optional_iso:
            self.log(f"Using optional ISO: {optional_iso_path}")
        ...
        try:
            ...
            if use_optional_iso:
                validate_iso_file(optional_iso_path)
            ...
            save_config(
                ...
                optional_iso_path=optional_iso_path,
            ...
            if use_optional_iso:
                qemu_cmd.extend(["-drive", f"file={optional_iso_path},media=cdrom,readonly=on"])
...
def execute_cli_flow():
    ...
    opt_iso_choice = input("Do you want to mount an additional optional ISO file? (yes/no): ").strip().lower()
    optional_iso_path = ""
    if opt_iso_choice in {"yes", "y"}:
        optional_iso_path = input("Enter the full path to your Optional ISO: ").strip()
        if not optional_iso_path:
            print("No optional ISO path entered. Exiting.")
            return
        if not os.path.exists(optional_iso_path):
            print(f"Optional ISO file not found: {optional_iso_path}")
            return
    ...
    if optional_iso_path:
        print(f"Using optional ISO: {optional_iso_path}")
        validate_iso_file(optional_iso_path)
